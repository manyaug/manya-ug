...
const VennSpotlightRenderer = ({
    ...
    return (
        <div className="flex flex-col h-full w-full bg-slate-50 overflow-hidden relative selection:bg-transparent">
            ...
            {/* 2. HUD AREA */}
            <div className="flex-none bg-white border-t border-slate-200 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] rounded-t-[32px] z-20 flex flex-col p-5 gap-4">
                <motion.div animate={errorAnim ? { x: [-10, 10, -10, 10, 0] } : {}} className="bg-fuchsia-50 border border-fuchsia-200 rounded-2xl p-4 text-center relative overflow-hidden shadow-sm">
                    <div className="absolute top-0 left-0 w-1.5 h-full bg-fuchsia-500" />
                    <div className="font-mono font-black text-fuchsia-900 tracking-wide mb-1 text-2xl uppercase">{question?.notation}</div>
                    <div className="text-slate-600 font-semibold text-sm">{question?.description}</div>
                </motion.div>
                ...
                <motion.button
                    onClick={handleCheck}
                    className={`w-full h-14 rounded-xl font-black flex items-center justify-center gap-2 uppercase tracking-wide transition-all ${
                        isResolved ? 'bg-indigo-600 text-white shadow-none' : 'bg-purple-600 text-white shadow-[0_4px_0_#6b21a8] active:translate-y-[4px] active:shadow-none'
                    }`}
                >
                    {isResolved ? (
                        currentStep < totalLevels - 1 ? <><ArrowRight size={20} /> NEXT QUESTION</> : <><CheckCircle2 size={20} /> QUEST COMPLETE!</>
                    ) : (
                        <><MousePointerClick size={20} strokeWidth={3} /> CHECK SHADING</>
                    )}
                </motion.button>
            </div>
        </div>
