...
export function loadAllProgress(subject) {
    if (!_progressCache[subject]) {
        try {
            const raw = storageService.getItem(getProgressKey(subject));
            _progressCache[subject] = raw ? JSON.parse(raw) : {};
        } catch {
            _progressCache[subject] = {};
        }
    }
    return _progressCache[subject];
}

export function saveAllProgress(subject, data) {
    _progressCache[subject] = data;
    // Non-blocking write to storage for offline bridging
    setTimeout(() => {
        try {
            storageService.setItem(getProgressKey(subject), JSON.stringify(data));
        } catch(e) {}
    }, 0);
}
...
export function trackWrongAnswer(subject, questionId) {
    ...
    const data = JSON.parse(storageService.getItem(key) || '{}');
...
export function setJustFinished(status) {
    storageService.setItem('manya_just_finished', JSON.stringify({
        ...status,
        timestamp: Date.now()
    }));
}
...
export function getJustFinished() {
    const raw = storageService.getItem('manya_just_finished');
    if (!raw) return null;
...
export function clearJustFinished() {
    storageService.removeItem('manya_just_finished');
}
