...
export default function QuestRunner() {
    ...
    // ΓöÇΓöÇ INITIAL BOOT ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    useEffect(() => {
    ...
    // ΓöÇΓöÇ ENGINE MOUNT HOOK ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    useEffect(() => {
        ...
        // ΓöÇΓöÇ DYNAMIC MODE INJECTION ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ CORE ORCHESTRATION BRIDGE ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const handleEngineResult = useCallback(async (result) => {
        if (!sessionRef.current) return;
        
        // ΓöÇΓöÇ Pulse Guard ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    const finishQuest = useCallback(() => {
        ...
        // 2. LOOT SYSTEM: SMART EVALUATION (Phase 2 ≡ƒÄü)
        ...
        earnedRewards.forEach(drop => {
            rewardService.grantChest(user.id, drop.chestType, drop.reason)
                ...
                .catch(e => console.warn('≡ƒÄü [QuestRunner] Chest grant failed:', e.message));
        });

        dispatch(incrementQuestCount());

        if (earnedRewards.length > 0) {
            dispatch(addToast({ message: `≡ƒÅå Quest complete! Earned ${earnedCoins} Coins and ${earnedRewards.length} Reward Chests!`, type: 'success' }));
        } else {
            dispatch(addToast({ message: `≡ƒÅü Quest complete! +${earnedCoins} Coins earned`, type: 'success' }));
        }

        // ≡ƒÄû∩╕Å POST-QUEST HOUSEKEEPING
        dispatch(checkAchievements());
        dispatch(syncUserData());

        // ≡ƒÅü Cloud Session Finalization (Schema Alignment ≡ƒ¢í∩╕Å)
        const sessionData = {
        ...
        // ≡ƒº¬ Respective Table Updates (Per Schema v5 ≡ƒ¢í∩╕Å)
        ...
        // ≡ƒöô UNLOCK PERSISTENCE: Explicitly track newly opened content (Phase 5 ≡ƒÜÇ)
        ...
        // ≡ƒÄ« SIMULATION PERSISTENCE (Phase 4 ≡ƒöô)
        if (hasFetcher) {
    ...
    return (
        <QuestBusProvider state={{
            ...
            <div className="quest-runner-shell" style={{ '--biome-color': biomeColor }}>
                {/* ΓöÇΓöÇ UNIFIED PREMIUM QUEST HUD ΓöÇΓöÇ */}
                {phase === 'running' && (
                    <QuestHUD 
