...
const CHEST_CONFIG = {
    bronze: { emoji: '🟫', name: 'Bronze Chest', color: '#cd7f32', glow: '#f4a460' },
    silver: { emoji: '🥈', name: 'Silver Chest', color: '#c0c0c0', glow: '#e8e8ff' },
    gold:   { emoji: '🥇', name: 'Gold Chest',   color: '#ffd700', glow: '#fff700' },
};

const REWARD_ICONS = { coins: '🪙', gems: '💎', xp: '⭐', unlock: '🔓', badge: '🏆' };
        ...
        <div className="chest-modal-overlay" role="dialog" aria-modal="true" aria-label="Chest Reward">
            <div className="chest-modal-card">
                ...
                <div className={`chest-emoji-wrap ${phase === 'shaking' ? 'shaking' : ''} ${phase !== 'closed' && phase !== 'shaking' ? 'opened' : ''}`}
                     style={{ '--glow': cfg.glow }}>
                    <span className="chest-emoji">{cfg.emoji}</span>
                    ...
                    <div className="chest-rewards-list">
                        {revealedRewards.map((r, i) => (
                            <div key={i} className="chest-reward-item reward-pop-in">
                                <span className="reward-icon">{REWARD_ICONS[r.type] || '✨'}</span>
                                <span className="reward-text">
                                    {r.amount ? `+${r.amount} ${r.type}` : r.value || r.type}
                                </span>
                            </div>
                        ))}
                    </div>
