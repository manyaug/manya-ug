export const SyntaxArchitect = {
    ...
    show: () => {
        ...
        if (window.QuestRunner) window.QuestRunner.saveStepProgress(s.index);
        const q = s.pool[s.index];
        s.container.innerHTML = `
            <div class="practice-box card-pop">
                <button class="rule-jump-btn" onclick="window.QuestRunner.jumpToRule()">💡 View Rule</button>
                <div class="bento-card q-card">
                    ...
                    <div class="stx-input-zone">
                        ...
                        <div class="stx-word-bank">
                            ${(q.options || []).map(o => `<button class="stx-pill" onclick="window.QuestRunner.SyntaxArchitect_fillInput('${o}')">${o}</button>`).join('')}
                        </div>
                    </div>
                </div>
            </div>`;
        ...
        if (inputEl) {
            inputEl.oninput = () => {
                window.QuestRunner.enableButton(inputEl.value.trim().length > 0, window.QuestRunner.SyntaxArchitect_check, "CHECK");
            };
            inputEl.onkeydown = (event) => {
                if (event.key === 'Enter' && !document.getElementById('main-action-btn').disabled) {
                    window.QuestRunner.SyntaxArchitect_check(); // Use the globally exposed function
                }
            };
        }

        // Expose fillInput and check to QuestRunner for global access from HTML
        if (window.QuestRunner) {
            window.QuestRunner.SyntaxArchitect_fillInput = SyntaxArchitect.fillInput;
            window.QuestRunner.SyntaxArchitect_check = SyntaxArchitect.check;
        }

        if (window.QuestRunner) window.QuestRunner.enableButton(false, window.QuestRunner.SyntaxArchitect_check, "CHECK"); // Initially disabled
    },
    ...
    fillInput: (option) => {
        ...
        if (input) {
            input.value = option;
            if (window.QuestRunner) window.QuestRunner.enableButton(true, window.QuestRunner.SyntaxArchitect_check, "CHECK");
        }
    },
    ...
    check: () => {
        ...
        if (correct) {
            ...
            if (window.QuestRunner) window.QuestRunner.enableButton(true, () => {
                ...
                if (window.QuestRunner) window.QuestRunner.enableButton(false, window.QuestRunner.SyntaxArchitect_check, "CHECK"); // Re-disable until new input
