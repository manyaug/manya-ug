export const HangmanEngine = {
    ...
    render: () => {
        ...
        let statusMessage = '';
        let buttonAction = () => { if (window.QuestRunner) window.QuestRunner.next(); };

        if (gameStatus === 'LOST') {
            statusMessage = `<span class="hangman-msg-lost">YOU LOST! The word was: ${s.currentWord}</span>`;
            if (window.QuestRunner) window.QuestRunner.enableButton(true, buttonAction, "CONTINUE");
    ...
    renderKeyboard: (status) => {
        ...
        return letters.split('').map(letter => {
            ...
            if (window.QuestRunner) window.QuestRunner.HangmanEngine_handleGuess = HangmanEngine.handleGuess;

            return `<button class="${className}" ${isDisabled ? 'disabled' : ''} onclick="window.QuestRunner.HangmanEngine_handleGuess('${letter}')">${letter}</button>`;
        }).join('');
    },
