export const WordGridEngine = {
    ...
    renderLabeling: async (container, data) => {
        ...
        if (window.QuestRunner) window.QuestRunner.WordGridEngine_handleStart = WordGridEngine.handleStart;
        ...
        document.removeEventListener('mousemove', window.QuestRunner?.WordGridEngine_moveHandler);
        document.removeEventListener('touchmove', window.QuestRunner?.WordGridEngine_moveHandler);
        
        if (window.QuestRunner) window.QuestRunner.WordGridEngine_moveHandler = moveHandler;
    ...
    handleEnd: () => {
        ...
        if (s.foundWords.size === s.wordsToPlace.length) {
            ...
            if (window.QuestRunner) window.QuestRunner.enableButton(true, null, `COMPLETE (+${s.score} Points)!`);
        }
    },
    ...
    render: () => {
        ...
        const gridHTML = s.grid.map((row, r) =>
            row.map((letter, c) => {
                ...
                const handlers = s.isGameOver ? '' : `
                    onmousedown="window.QuestRunner.WordGridEngine_handleStart(${r}, ${c});"
                    ontouchstart="window.QuestRunner.WordGridEngine_handleStart(${r}, ${c});"
                `;
        ...
        s.container.innerHTML = `
            <div class="wordgrid-box card-pop">
                ...
                ${s.hint ? `<div class="grid-hint" style="background:#F0F9FF; border-left:4px solid #3B82F6; padding:12px; border-radius:10px; margin-top:15px; font-size:12px; font-weight:700; color:#1E3A8A; width:100%;">💡 HINT: ${s.hint}</div>` : ''}
            </div>`;
    },
