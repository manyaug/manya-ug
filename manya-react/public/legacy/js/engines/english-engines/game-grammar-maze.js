export const JungleMazeEngine = {
    ...
    render: (rows, cols, mazeData, currentLevel) => {
        ...
        s.container.innerHTML = `
            <div id="jungle-quest-shell">
                ...
                ${s.hint ? `<div class="maze-hint" style="background:#F0FDF4; border-left:4px solid #22C55E; padding:12px; border-radius:10px; margin: 15px auto; font-size:12px; font-weight:700; color:#14532D; width:90%; max-width:400px; text-align:center; z-index:30;">💡 HINT: ${s.hint}</div>` : ''}
            </div>
        `;
    },
    ...
    renderLabeling: (container, data) => {
        ...
        JungleMazeEngine.state.hint = data.hint || "";
        // Fix for missing image: fallback to Manya icon or emoji if icon missing
        JungleMazeEngine.state.playerIcon = window.QuestRunner?.CHAR_ICONS?.manya || "assets/images/manya_icon.png";
