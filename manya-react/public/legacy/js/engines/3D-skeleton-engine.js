    ...
    injectStyles: () => {
        ...
        style.innerHTML = `
            .skel-root { 
                ...
                width: 100%;
                height: 100%;
                background: ${isQuest ? 'transparent' : 'var(--bg-main, #FDFBF7)'}; 
                display: grid;
...
/* ... rest of existing styles ... */
    ...
    renderStudy: async (container, data) => {
        ...
        container.innerHTML = `
            <div class="skel-root animate-in" data-theme="light">
                <!-- Content -->
    ...
    renderLabeling: async (container, data) => {
        ...
        window.HandleSkelQuiz = (btn, word) => {
            if (!currentPinId) {
                if(window.addToast) window.addToast({message: "Select a pulsing pin first!", type: "info"});
                return;
            }
            const hs = hotspots.find(h => h.id === currentPinId);
            if (hs.label.toLowerCase() === word.toLowerCase()) {
                if(window.addToast) window.addToast({message: "🌟 Great Work Hero!", type: "success"});
                ...
                // Finish condition
                if (container.querySelectorAll('.Hotspot:not(.correct-pin)').length === 0) {
