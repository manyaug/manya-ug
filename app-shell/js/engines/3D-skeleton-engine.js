/**
 * MANYA 3D SKELETON ENGINE (v11.0 - THE UNABRIDGED MASTER)
 * --------------------------------------------------------
 * This is the full-fat, non-truncated technical implementation.
 * 
 * CORE SYSTEMS:
 * 1. MODE ROUTER: Deeply forks UI for 'study' vs 'labeling'.
 * 2. VECTOR ENGINE: Maps 'pos' and 'norm' strings to 3D surface coordinates.
 * 3. CAMERA DIRECTOR: Uses trigonometry to rotate camera to face bone normals.
 * 4. DRAWER SYSTEM: Premium slide-up instructional panel for Study mode.
 * 5. WORD BANK: Randomized button grid for Labeling mode.
 */
...
export const SkeletonQuestEngine = {
    ...
    injectStyles: () => {
        ...
        style.innerHTML = `
            ...
            .HotspotAnnotation {
                ...
                width: max-content; box-shadow: 0 4px 15px rgba(0,0,0,0.1);
                opacity: 0; pointer-events: none; transition: 0.2s;
                font-size: 11px; font-weight: 900; color: #1E293B; border: 1px solid #F1F5F9;
            }
            .Hotspot.selected .HotspotAnnotation { opacity: 1; top: -52px; }

            /* --- SLIDE-UP INFO DRAWER (STUDY MODE) --- */
            .info-drawer {
    ...
    // --- 2. CAMERA CALCULATION (THE POWER TILT) ---
    // Converts Normals into camera theta/phi spherical angles
    calculateOrbit: (normStr) => {
        ...
        return `${theta}deg ${phi}deg 80%`; // Target 80% zoom
    },

    // --- 3. RENDER STUDY MODE (The Detailed Sim) ---
    renderStudy: (container, data) => {
                ...
                <div class="set-game-card">
                    ...
                    <div class="info-drawer" id="study-drawer">
                        <div class="drawer-header">
                            <span class="drawer-title" id="d-title">Bone Detail</span>
                        ...
                        <div class="drawer-content" id="d-content">
                            ...
                            ${data.notes ? `<ul style="padding-left:20px;">${data.notes.map(n => `<li style="margin-bottom:8px;">${n}</li>`).join('')}</ul>` : ''}
                            <div style="background:#F1F5F9; padding:15px; border-radius:15px; font-size:12px; font-weight:800; color:#64748B; text-align:center; margin-top:20px;">
                                👆 Tap a purple pin on the skeleton to explore!
                            </div>
                        </div>
                    </div>
                </div>
        ...
        // Wait for model to prevent bunched pins
        ...
        container.querySelectorAll('.Hotspot').forEach(pin => {
            pin.onclick = (e) => {
                ...
                container.querySelector('#d-content').innerHTML = `
                    ...
                    ${hsData.examTip ? `<div class="exam-tip-box"><strong>💡 PLE INSIGHT</strong><br>${hsData.examTip}</div>` : ''}
                `;
    ...
    // --- 4. RENDER LABELING MODE (Interactive Quiz) ---
    renderLabeling: (container, data) => {
        ...
        container.querySelectorAll('.Hotspot').forEach(pin => {
            pin.onclick = () => {
                ...
                viewer.cameraTarget = pin.dataset.position;
                viewer.cameraOrbit = SkeletonQuestEngine.calculateOrbit(pin.dataset.normal);
                document.getElementById('q-status').innerText = "What is the name of this part?";
            };
        });

        window.ManyaQuiz3D = (btn, word) => {
            if (!currentPinId) return document.getElementById('q-status').innerText = "⚠️ Select a pin first!";
